...
curl -XPUT "http://localhost:9200/customers2" -H "Content-Type: application/json" -d '{"settings":{"number_of_shards":3,"number_of_replicas":2,"auto_expand_replicas":false}}'
...
curl -XPOST 'http://localhost:9200/customers2/_bulk' -H "Content-Type: application/json" --data-binary @datasets/customers2_compact.json
...
curl -XGET "http://localhost:9200/customers2/_count" \
     -H "Content-Type: application/json" \
     -d '{
       "query": {
         "match_all": {}
